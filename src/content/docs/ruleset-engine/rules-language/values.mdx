...
The following example is based on the [`http.request.headers`](ruleset-engine/rules-language/fields/http-request-header/) field with a data type of `Map<ArrayString>>`, where array elements are of `String` data type.
...
any(http.request.headers["accept"][*] == "application/json") # ==> false
any(http.request.headers["accept"][*] == "text/plain")       # ==> true
...
- For IP addresses, you can enter:
  ...
  - CIDR ranges (for example, `193.0.2.0/24` or `2001:0db8::/32`).
...
ip.src in {198.51.100.1 198.51.100.3..198.51.100.7 193.0.2.0/24 2001:0db8::/32}
